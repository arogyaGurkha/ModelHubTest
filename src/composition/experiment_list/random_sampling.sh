...
for (( i=0; i<$steps; i++ ))
do
  cmd="python $PYTHON_FILE --desc=\"Testing for random sampling\" --root_dir=\"$ROOT_DIR\" --is_gt=False --n_samples=$n_samples --batch_processing=True --m_samples=26 --runs=15 --gpu_id=0 --sampling=\"random\""
  echo $cmd
  echo "--------------------------------------------------"
  eval $cmd
